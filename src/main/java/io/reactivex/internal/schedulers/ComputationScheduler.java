...
public final class ComputationScheduler extends Scheduler {
    ...
    static {
        ...
        THREAD_FACTORY = new RxThreadFactory(THREAD_NAME_PREFIX, priority, true);//不推荐线程堵塞
    ...
    static final class FixedSchedulerPool {
        ...
        final PoolWorker[] eventLoops;//worker线程列表,即提前创建好用于高密度计算的线程池
        long n;
    ...
    public Disposable scheduleDirect(@NonNull Runnable run, long delay, TimeUnit unit) {
        PoolWorker w = pool.get().getEventLoop();//获得线程池中的一个worker
        return w.scheduleDirect(run, delay, unit);//使用worker调度
    }
    ...
    public Disposable schedulePeriodicallyDirect(@NonNull Runnable run, long initialDelay, long period, TimeUnit unit) {
        PoolWorker w = pool.get().getEventLoop();//获得线程池中的一个worker
        return w.schedulePeriodicallyDirect(run, initialDelay, period, unit);//使用worker调度
    }
